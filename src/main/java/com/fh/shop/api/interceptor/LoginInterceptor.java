...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,Object handler) throws Exception {
        ...
        //判断用户信息是否被篡改[验签]
        String base64MemberJson = split[0];
        String signBase64 = split[1];
        //重新生成新的签名和客户端传递过来的签名进行对比
        String newSign = Md5Util.sign(base64MemberJson,Md5Util.SECRET);
        ...
        //判断是否超时[redis]
        //通弄个new String(字节数组,编码格式);将字节转为字符串
        String memberJson = new String(Base64.getDecoder().decode(base64MemberJson),"utf-8");
