        ...
        if (StringUtils.isNotEmpty(cartJson)){
           ...
           if (cartItem != null){
               ...
               if (num1 <= 0){
                   ...
                   //【特别注意：倒着删除，通过iterator删除】再循环遍历list的时候，删除集合中的信息
               ...
               updateCart(memberId, cart);
                ...
                //如果商品不存在添加商品,更新购物车[总个数,总计]
               if (num <= 0){
                   return ServerResponse.error(ResponseEnum.CARR_NUM_IS_ERROR);
               }
               //构建商品
               ...
               BigDecimal sunPrice = BigDecimalUtil.mul(num+"",product.getPrice().toString());
               cartItemInfo.setSubPrice(sunPrice);
